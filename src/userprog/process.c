...
//		printf("DEBUG: load start [%s]\n", file_name);
  ...
  /* REQUIREMENT : parse arguments */
  char temp_filename[1024];
  char *argv[32];
  strlcpy(temp_filename, file_name, strlen(file_name)+1);
  size_t argc = parse_filename(temp_filename, argv);
  ...
  /* REQUIREMENT : construct stack */
  construct_stack(argc, argv, esp);
/*
  uintptr_t ofs = (uintptr_t)*esp;
  hex_dump(ofs, *esp, (uint32_t)PHYS_BASE - (uint32_t) *esp, true);
*/
  /* Start address. */
...
size_t 
parse_filename(char *origin, char *argv[32])
{
	char *token, *next;
	size_t argc = 0;

	token = strtok_r(origin, " ", &next);
	while(token != NULL){
		argv[argc++] = token;
...
void
construct_stack(size_t argc, char* argv[32], void** esp)
{
	char **addr;
	int start = (int)*esp;
	...
	int interval = (start - (int)(*esp)) % 4;
	if(interval != 0){
		for(int j=0;j<4 - interval; j++){
			(*esp) -= 1;
			memset(*esp, 0, 1);
		}
	}
